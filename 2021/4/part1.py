...
while winner == False:
	thisNumber = numbers[numPos]
	print("Picked ", thisNumber)

	# i believe that since im working with complex obs
	# im passing by ref so the return isn't really necessary, but...
	boards = updateBoards(boards, thisNumber)
	
	for board in boards:
		if isWinner(board):
			print("WINNER!")
			score = getScore(board, thisNumber)
			pp.pprint(board["rows"])
			print("Final Score", score)
			winner = True

	numPos += 1
	if numPos == len(numbers):
